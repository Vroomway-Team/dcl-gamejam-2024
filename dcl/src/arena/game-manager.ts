...
export module GameManager {
    ...
    export function Initialize() {
        ...
        LobbyLabel.PlayerClaimCallback = function(vehicle:number) {
            ...
            //send claim request
            Networking.ClientRoom.send("lobby-join", {id:Networking.GetUserID(), displayName:Networking.GetUserName(), vehicle:vehicle});
        }
        ...
        LobbyLabel.PlayerUnclaimCallback = function() {
            ...
            if(Networking.ClientRoom == undefined) return;
            //send claim request
            Networking.ClientRoom.send("lobby-leave", {id:Networking.GetUserID()});
        }
    ...
    export function SetGameState(state:GameState.GAME_STATE_TYPES) {
        ...
        switch(GameState.CurGameState.GetValue()) {
            ...
            case GameState.GAME_STATE_TYPES.PLAYING_IN_SESSION:
                ...
                movePlayerTo({
                    newRelativePosition: {x:32, y:8, z:32},
                    cameraTarget: {x:32, y:8, z:40},
                });
            break;
    ...
    GameState.LobbyStartCountdown.RegisterCallback(
        function(value:number) {
            ...
            TextShape.getMutable(entityCountdown).text = value.toString();
        }
    );
    ...
    const buttonGameEnd = new DynamicButton_Simple(1, {x:32,y:8.5,z:32}, "FORCE: END");
