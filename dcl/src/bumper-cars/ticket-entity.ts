...
import { Dictionary, FunctionCallback, FunctionCallbackIndex, List, TRANSFORM_SCALE_OFF, TRANSFORM_SCALE_ON, TransformDataObject } from "../utilities/escentials";
...
export module TicketEntity {
    ...
    export class TicketEntityObject {
        ...
        public Initialize(data:TicketEntityObjectCreationData) {
            ...
            transform.position = {
                x: ((Math.random()-0.5)*RANDOM_OFFSET)+MODEL_OFFSET.x,
                y: MODEL_OFFSET.y,
                z: ((Math.random()-0.5)*RANDOM_OFFSET)+MODEL_OFFSET.z
            };
            if(data.transform.position) {
                transform.position.x = transform.position.x + data.transform.position.x;
                transform.position.y = transform.position.y + data.transform.position.y;
                transform.position.z = transform.position.z + data.transform.position.z;
            }
            ...
            utils.triggers.addTrigger(
                ...
                //enter callback
                function() { TicketSpawnManager.ClaimTicket(data.index); },
                //exit callback
                undefined
            );
